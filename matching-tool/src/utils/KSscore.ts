...
const scoreGender = (tutor: Tutor, tutee: Tutee): number => {
  ...
  return String(tutor.personalData.gender) == String(tutee.preferedGender)
    ? 1
    : 0;
};
...
const scoreProBono = (tutor: Tutor, tutee: Tutee): number => {
  ...
  if (tutor.isProBonoOk === undefined || tutee.isOnFinancialAid === undefined)
    return 0.5;
  return 0;
};
...
const scoreFinancialAid = (tutor: Tutor, tutee: Tutee): number => {
  ...
  if (tutor.isUnaidedOk === undefined || tutee.isOnFinancialAid === undefined)
    return 0.5;
  return 0;
};
