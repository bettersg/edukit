...
export type DataFormat = // Shared parameters
  ({
    fieldName: string;
  } & (
    | {
  ...
  public compileColumnHeader() {
    ...
    for (let [i, formatRule] of this.format.entries()) {
      if (
        ...
        formatRule.columnKeywords.length > 0 &&
        formatRule.getterType == 'cell_value'
      ) {
        if (!('multipleColumns' in formatRule) || !formatRule.multipleColumns) {
            ...
            if ('columnKeywords' in formatRule) {
              for (let item of formatRule.columnKeywords) {
                if (item.startsWith('!')) {
                  found =
                    found &&
                    !header.toLowerCase().includes(item.slice(1).toLowerCase());
                } else {
                  found =
                    found && header.toLowerCase().includes(item.toLowerCase());
                }
              }
            }
            return found;
          ...
          if (headerIndex == -1) {
            alert(
              `Incorrect Data Header Format! Didnt Find - ${formatRule.fieldName}`,
            );
            throw new Error(
              `Column with keywords ${formatRule.columnKeywords} not found for format rule ${formatRule.fieldName}`,
            );
          }
          ...
          if (headerIndices.length < 1) {
            ...
            throw new Error(
              `Columns with keywords ${formatRule.columnKeywords} not found for format rule ${formatRule.fieldName}`,
            );
          } else {
      ...
      for (let formatRule of this.format) {
        if (formatRule.getterType == 'row_index') {
          if (formatRule.type !== 'number') {
            throw new TypeError(
              'Row index can only be used in a number field',
              { cause: { code: 'ROW_INDEX_NOT_NUMBER' } },
            );
          }
          ...
          if (formatRule.type == 'number') {
              ...
              if (formatRule.filter?.params instanceof Array) {
                ...
                if (!found) {
                  dataRow[formatRule.fieldName] =
                    formatRule.filter?.noMatchValue;
                }
              } else if (
                formatRule.filter?.params.constructor.name === 'Object'
              ) {
                let filter = formatRule.filter.params;
                let found = false;
            ...
            if (formatRule.multipleColumns) {
              let raw: string[] = formatRule.columnIndex!.map(index =>
                String(row[index]),
              );
              if ('filter' in formatRule) {
                if (formatRule.filter?.params instanceof Array) {
            ...
            if (formatRule.multipleColumns) {
              ...
              if ('mapping' in formatRule) {
                if (!(formatRule.mapping instanceof Array)) {
                  ...
                  for (let arr of raw) {
                    for (let item of arr) {
                      let toPush: (string | number)[] = Object.keys(
                        formatRule.mapping!,
                      )
                        .filter(key =>
                    ...
                    if (filterItem.startsWith('|')) {
                      found =
                        ...
                        raw
                          .toLowerCase()
                          .trim()
                          .includes(filterItem.slice(1).toLowerCase());
                    } else {
                      found =
                        found &&
                        raw
